...
$(document).ready(function () {
  $(".filter-nav button").on("click", function () {
    ...
    const items = $(".portfolio-card"); // Chọn đúng phần tử có data-category

    // Xóa class active khỏi các button khác
    $(this).addClass("active").siblings().removeClass("active");

    items.each(function () {
      const category = $(this).attr("data-category");
      if (filter === "" || category === filter) {
        $(this).parent().removeClass("d-none").addClass("animate__animated animate__fadeIn");
      } else {
        $(this).parent().addClass("d-none").removeClass("animate__animated animate__fadeIn");
      }
    });
  });
});
